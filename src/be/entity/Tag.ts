...
Tag.prototype.get = async ({
  ...
  const query = new DataSource(ormConfig).manager.connection.getRepository(Tag)
  ...
  try {
    ...
    if (getTagsForFolder) {
      const selectedFolder = await new DataSource(ormConfig).manager.connection.getRepository(Folder).findOne({
        where: [ {FolderLocation : folderLocation}],
        relations: {
            Category: true,
            Language: true
        }
...
Tag.prototype.create = async (
  ...
  const tagTypes = await new DataSource(ormConfig).manager.connection.getRepository(TagType)
    .createQueryBuilder()
  ...
  const manager = transactionManager || new DataSource(ormConfig).manager;
...
Tag.prototype.modifyTagsOfFolders = async (
  ...
  const insertFoldersQuery = new DataSource(ormConfig).manager.connection.getRepository(Folder)
  ...
  if (category === null) newCategory = null;
    ...
    newCategory = await new DataSource(ormConfig).manager.connection.getRepository(Category)
      .createQueryBuilder()
      .whereInIds(category)
      .getOne();
  ...
  if (language === null) newLanguage = null;
    ...
    newLanguage = await new DataSource(ormConfig).manager.connection.getRepository(Language)
      .createQueryBuilder()
      .whereInIds(language)
      .getOne();
  ...
  const updateFoldersTag = async (newlyCreatedTags: Tag[]) => {
    ...
    const foundTags = await new DataSource(ormConfig).manager.connection.getRepository(Tag)
      .createQueryBuilder()
      .whereInIds(tagIds)
      .getMany();
  ...
  try {
    await new DataSource(ormConfig).manager.transaction(async transactionManager => {
...
Tag.prototype.calculateRelations =
  async (): Promise<TagRelationQueryResult> => {
    const rawData = await new DataSource(ormConfig).manager.connection.getRepository(Folder)
      .createQueryBuilder('folder')
    ...
    const findFrequentTags = async (source: Record<string, string[]>) => {
      const getSourceWithFrequentTags = async () => {
        ...
        for (const [parentTag] of Object.entries(newSource)) {
          ...
          const numberOfFoldersWithParentTag = await new DataSource(ormConfig).manager.connection.getRepository(Folder)
            .createQueryBuilder('folder')
...
Tag.prototype.clear = async (): Promise<QueryResult> => {
  const unusedTags = await new DataSource(ormConfig).manager.connection.getRepository(Tag)
    .createQueryBuilder('tag')
  ...
  try {
    const manager = new DataSource(ormConfig).manager;
...
Tag.prototype.removeAllTagsFromFolders = async ({
  ...
  const selectedFolders = await new DataSource(ormConfig).manager.connection.getRepository(Folder)
    .createQueryBuilder()
    .whereInIds(folderLocations)
    .getMany();
  ...
  try {
    const manager = new DataSource(ormConfig).manager;
    await manager.save(selectedFolders);
    return {
      message: MESSAGE.SUCCESS,
      status: StatusCode.Success
    };
...
Tag.prototype.updateTags = async (
  ...
  for (const tag of updatedTags) {
    ...
    const query = new DataSource(ormConfig).manager.connection.getRepository(Tag).createQueryBuilder('tag');
  ...
  const mergeTags = async (
    ...
    const affectedFolders = await new DataSource(ormConfig).manager.connection.getRepository(Folder)
      .createQueryBuilder('folder')
      .innerJoinAndSelect('folder.Tags', 'tag')
      .where(query => {
