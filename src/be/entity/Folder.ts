...
export default class Folder {
  ...
  isExisting = async (folderLocation: string): Promise<boolean> => {
    let folder = new DataSource(ormConfig).manager.connection.getRepository(Folder).findOne({
...
Folder.prototype.get = async (
  ...
  const query = new DataSource(ormConfig).manager.connection.getRepository(Folder)
  ...
  _.forEach(tags, (value:any, key:any) => {
    switch (key) {
      case 'name':
...
Folder.prototype.import = async (
  ...
  const manager = new DataSource(ormConfig).manager;
  const folderRepository = new DataSource(ormConfig).manager.connection.getRepository(Folder);
  const tagRepository = new DataSource(ormConfig).manager.connection.getRepository(Tag);
  const allCategories = await new DataSource(ormConfig).manager.connection.getRepository(Category).find();
  const allLanguages = await new DataSource(ormConfig).manager.connection.getRepository(Language).find();
  const allTagTypes = await new DataSource(ormConfig).manager.connection.getRepository(TagType).find();
  ...
  const getTagInDB = async (tagType: string, tagName: string) => {
    return await tagRepository.findOne({
      where: [{TagId: getTagId(tagType, tagName)}]
    });
  };
      ...
      const getFolderTags = async () => {
        for (const tagType of Object.keys(transferTags)) {
          ...
          for (const tagName of transferTags[tagType]) {
            ...
            if (tagAlreadyInDB) {
              folderTags.push(tagAlreadyInDB);
            }
          }
        }
      };
...
Folder.prototype.export = async (): Promise<QueryResult> => {
  try {
    const allFolders = await new DataSource(ormConfig).manager.connection.getRepository(Folder)
      .createQueryBuilder('folder')
...
Folder.prototype.rename = async (
  ...
  const manager = new DataSource(ormConfig).manager;
  const oldFolder = await new DataSource(ormConfig).manager.connection.getRepository(Folder)
    .createQueryBuilder('folder')
    .leftJoinAndSelect('folder.Category', 'category')
    .leftJoinAndSelect('folder.Language', 'language')
...
Folder.prototype.remove = async (
  ...
  try {
    await new DataSource(ormConfig).manager.connection.getRepository(Folder).delete(removedFolders);
    return {
      message: MESSAGE.SUCCESS,
      status: StatusCode.Success
    };
